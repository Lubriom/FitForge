<template>
  <div class="w-full h-full p-6 p-y-10 flex justify-center items-center">
    <div class="w-4/5 h-fit p-5 bg-white rounded-2xl shadow">
      ...
      <h2 class="text-2xl font-bold mb-4 text-center" v-else>Plan de Entrenamiento</h2>
...
const submitForm = async () => {
  try {
    ...
    router.push({ name: "ExercisesTrainEdit" , params: { id: response.data.id, usuarioId: response.data.usuarioId } }); 
  } catch (error) {
    console.error(error);
